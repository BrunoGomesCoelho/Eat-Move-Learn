...
class MultiOpponentWrapper(gym.Wrapper):
...
class GlobalRewardWrapper(gym.Wrapper):
    ...
    def step(self, action):
        ...
        if not done:
            ...
            # TODO: if I won, give me da usual reward
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        type=str,
        default='',
        help="Reload previously trained model to continue training (or do something else maybe).",
    )
    args = parser.parse_args()
    LOAD_FILE = args.load_file
    if LOAD_FILE:
        LOAD_FILE = os.path.join('logs', args.load_file + '.zip')
    ...
    if args.ppo:
        if LOAD_FILE:
            trainer = PPO.load(LOAD_FILE)
        else:
            # coment **params for default parameters
            trainer = PPO(policy='MlpPolicy'
                          , env=m_env
